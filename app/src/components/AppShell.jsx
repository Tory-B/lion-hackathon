...
export default function AppShell({ children, crumb }) {
  ...
  const questionnaireCount = analyses.filter((a) => a.status === 'COMPLETED').length
      ...
      <div className="max-w-[1400px] mx-auto flex">
        <aside className="hidden md:block w-[248px] shrink-0 px-6 py-6">
          ...
          <nav className="flex flex-col gap-0.5 mb-6">
            ...
            <span className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-[#c3c9c5]">
              아이템 비교
              <span className="text-[10px] border border-[#e2e6e3] rounded px-1.5 py-0.5">BETA</span>
            </span>
          </nav>

          <p className="text-[11px] font-medium text-[#9aa39e] mb-2 px-1">설문</p>
          <nav className="flex flex-col gap-0.5 mb-6">
            <span className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-[#4b5450]">
              설문지
          ...
          <nav className="flex flex-col gap-0.5">
            <span className="px-3 py-2 rounded-lg text-sm text-[#4b5450]">리포트</span>
          </nav>
        </aside>
